...
`;
  ...
  async *getTableInsert(tableName) {
    ...
    if (rowCount > 0) {
      ...
      const columnNames = notGeneratedColumns.map(
        (col) => "`" + col.name + "`"
      );
      ...
      for await (const row of stream) {
        ...
        for (const i in notGeneratedColumns) {
          ...
          if (val === null) sqlInsertString += "NULL";
            ...
            sqlInsertString += moment(val).isValid()
              ? this.escapeAndQuote(
                  moment(val).format(`YYYY-MM-DD HH:mm:ss${datePrecision}`)
                )
          ...
          else if (NUMBER.includes(column.type)) sqlInsertString += val;
            ...
            } else geoJson = this._getGeoJSON(val);

            sqlInsertString += `ST_GeomFromGeoJSON('${JSON.stringify(
              geoJson
            )}')`;
  ...
  getMarkers(points) {
    if (Array.isArray(points)) {
      if (getArrayDepth(points) === 1)
        ...
        return polygon(
          points.map((arr) =>
            arr.reduce((acc, curr) => [...acc, [curr.x, curr.y]], [])
          )
        );
  ...
  async getFunctions() {
    ...
    for (const func of functions) {
      ...
      sqlString += await this.getRoutineSyntax(func.Name, func.Type, definer);
    }
  ...
  _getGeoJSON(val) {
    if (Array.isArray(val)) {
      if (getArrayDepth(val) === 1)
        return lineString(
          val.reduce((acc, curr) => [...acc, [curr.x, curr.y]], [])
        );
      else
        return polygon(
          val.map((arr) =>
